...
const PredictionResults = ({ data, onClose, inline = false }) => {
  ...
  return (
    ...
    <div ref={resultsRef} className="w-full h-screen">
      {/* Close Button */}
      ...
      <div className="w-full  bg-black mt-8">
        ...
        <div className={`flex items-center justify-center rounded-t-3xl`}>
          <div className="w-[90%] flex items-center justify-between flex-wrap gap-4 py-5 mt-16">
            <div >
            ...
            <div className={`rounded-2xl backdrop-blur-sm`}>
              <div className="flex items-center gap-3">
                <p className={`text-3xl font-bold ${signalColor} mb-1`}>{fused_signal.signal}</p>
                <div className='h-12 w-1 bg-white'></div>
                <div className='flex flex-col items-center'>
                  <p className=" text-gray-400 text-md">Trading Signal</p>
                  <p className={`text-xs text-gray-400 ${signalColor}`}>{fused_signal.strength} Strength</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col items-center p-8 space-y-6">
          ...
          <div className='w-[60%] mt-5 flex flex-col gap-3'>
            <div className="flex justify-center items-center gap-5">
              <span className="text-xl poppins text-white">Confidence Level</span>
              <span className='w-5 h-0.5 bg-white'></span>
              <span className="text-xl poppins text-white">
                {(direction_prediction.confidence * 100).toFixed(2)}%
              </span>
            </div>

            <div className="bg-gray-200 rounded-full h-4 overflow-hidden border">
              <div
                className={`h-full transition-all duration-1000 ${isPredictionIncrease ? 'bg-gradient-to-r from-[#2a7a02] to-[#5cfc0c]' : 'bg-gradient-to-r from-red-600 to-red-400'}`}
            ...
            <div className='flex items-center justify-between gap-5 px-8'>
              ...
              <span className={`text-xl poppins ${isPredictionIncrease ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className='w-[60%] mt-5 flex flex-col gap-3'>
            <div className="flex justify-between items-center gap-5">
              <div className='text-green-500 text-lg poppins'>{(sentiment_analysis.positive_ratio * 100).toFixed(0)}%</div>
              <div className='text-yellow-500 text-lg poppins'>{(sentiment_analysis.neutral_ratio * 100).toFixed(0)}%</div>
              <div className='text-red-500 text-lg poppins'>{(sentiment_analysis.negative_ratio * 100).toFixed(0)}%</div>
            </div>

            <div className="bg-gray-200 rounded-full h-4 overflow-hidden border flex">
              <div
                className="bg-green-600 h-full"
                style={{ width: `${sentiment_analysis.positive_ratio * 100}%` }}
              />
              <div
                className="bg-yellow-500 h-full"
                style={{ width: `${sentiment_analysis.neutral_ratio * 100}%` }}
              />
              <div
                className="bg-red-800 h-full"
                style={{ width: `${sentiment_analysis.negative_ratio * 100}%` }}
              />
            </div>
            <div className='flex items-center justify-between poppins mt-4'>
              <div className='text-white text-xl ml-5'>
                Market Sentiment (<span className="text-xl font-bold text-blue-400">
                  {sentiment_analysis.news_analyzed}
                </span>)
              </div>
              <span className={`text-xl mr-30 ${isPredictionIncrease ? 'text-green-400' : 'text-red-400'}`}>
                {sentiment_analysis.sentiment_score.toFixed(4)}
              </span>
              <span className={`text-xl font-bold mr-20 ${sentimentColor}`}>
            ...
            <div className="bg-gradient-to-br from-gray-800/80 to-gray-900/80 border-2 border-gray-700 rounded-2xl p-6">
              ...
              <div className="bg-gray-900/50 border border-gray-700 rounded-xl p-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  ...
                  <span className={`inline-block px-4 py-1.5 rounded-full text-sm font-bold ${reasoning.sentiment_context.impact === 'Positive' ? 'bg-green-500/20 text-green-400 border border-green-500/50' :
                ...
                {announcements.map((announcement, index) => (
                  <div key={index} className="bg-gray-900/50 border border-gray-700 hover:border-indigo-500/50 rounded-xl p-5 transition-all hover:shadow-lg hover:shadow-indigo-500/10">
                    <div className="flex justify-between items-start gap-4 mb-3">
                      ...
                      <span className={`px-3 py-1.5 rounded-full text-sm font-bold whitespace-nowrap ${announcement.sentiment === 'Positive' ? 'bg-green-500/20 text-green-400 border border-green-500/50' :
